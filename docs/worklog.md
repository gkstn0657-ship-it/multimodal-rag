...
- IMAGE 경로 임베딩 텍스트 문제 규명(합성 질의 194건, v2 캡션 단독이 OCR보다 약함) 및
  전사 우선 캡션 v3 + 캡션·OCR 합집합으로 수정 (D-15) — 997장 재캡셔닝은 진행 중 중단(129/997)
...
1. **SDS KoPub v3 재캡셔닝 재개.** 997장 중 129장에서 중단됨. 마저 끝내고 재임베딩 후 합성
   질의(194) + SDS QA(600)로 v1+OCR 조합(53.1%/0.408, D-15) 대비 개선 여부를 재확인한다.
2. **차트가 있는 TEXT 경로 페이지.** D-14의 픽셀 휴리스틱은 기각됐다. DART는 원본 PDF라
   `fitz`의 `get_drawings()`/`get_images()`로 실제 벡터 그래픽 객체를 뽑아 시도해볼 수 있는
   실질적 대상이다 — 아직 미착수.
3. **기권 게이트.** D-13에서 자기모순은 고쳤지만 그 대가로 검색 실패 시에도 기권 대신 무관한
   문서로 답하는 사례가 늘었다(7번 질문). 리랭크 점수 기반 기권 임계값 설계·구현 필요.
4. **DART 코퍼스 자체 평가.** 인덱싱은 끝났지만 정량 평가(질의셋)는 아직 없다. 규모가 작아
   (8,418페이지) 라벨링 질의셋을 직접 만들어야 한다.
5. **README·다이어그램 마무리.** 전체 아키텍처 다이어그램, 포트폴리오 최종 정리.
