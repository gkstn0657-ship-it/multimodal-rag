...
1. **기권 게이트.** D-20·D-21 모두 DART 무관 비율 0.62를 못 줄였다 — "같은 회사, 다른 주제"는 문서 단위
   신호(하이브리드·접두어)로 못 풀고 페이지 단위 관련성 판단이 필요하다. 판정 평가의 리랭크 점수 분포로
   임계값을 정한다. D-13 부작용(검색 실패 시 무관 문서로 답변)도 같이 해결.
