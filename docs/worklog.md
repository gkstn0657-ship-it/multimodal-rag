...
1. **기권 게이트 — 후보 규칙 검증.** 단일 신호 3종은 기각(D-22), 조합 규칙 `top-5 문서 종류 ≥ 4 AND
   1·2위 격차 < 0.02`가 과잉기권 1/17·포착 3/5로 유력하지만 양성 표본이 4~5건이라 미채택. 답 없는 질의
   20~30개를 추가 제작해 k·m을 재선정하고, 과잉기권 ≤10%·포착 ≥70%면 `answer.py`에 도입. 대안으로
   생성 후 사후 검증 방식도 검토. D-13 부작용(검색 실패 시 무관 문서로 답변)은 그때까지 남는다.
