...
|---|---|---|
| 평균 | 43.5 | 48.1 |
| text / cross / visual | 45.2 / 40.8 / 44.4 | 57.6 / 40.8 / 45.8 |
| 승 / 패 / 동률 | 5 / 7 / 3 | |
...
| 퇴행 출력 `@@@@…` | 14번. 입력 변형 11종 전부 재현, 컨트롤 단어까지 퇴행, 텍스트 모델은 정상 → VL 러너 상태 손상 | `ollama stop` 재로드로 회복 확인. 퇴행 감지·자동 재시작·재시도 추가 (D-12). 14번 0점은 인프라 장애로 재분류 |
...
커밋 33개. 각 단계는 `feat` / `fix` / `perf` / `docs` / `test` / `chore` 접두어로 나뉘어 있다.
...
1. **생성 단계 잔여 결함.** (a) 14번 `@@@@` 퇴행 출력 원인 규명, (b) 답 뒤에 "찾을 수 없습니다"를
   덧붙이는 자기모순 — 시스템 프롬프트의 기권 지시 조정, (c) D-11 수정 후 15문항을 temperature 0으로
   재채점해 재현 가능한 기준선 확보. 채점 자체는 이번처럼 이 세션의 Claude가 맡으면 API 키 없이 가능하다.
2. **차트가 있는 TEXT 경로 페이지.** 11번처럼 텍스트가 100자 이상이라 이미지 경로로 안 가는 차트 페이지는
   VLM이 그래프 값을 볼 수 없다. 페이지 내 그림·표 영역 감지(잉크 밀도 분포, 큰 래스터 블록)를 라우팅
   조건에 추가해야 한다.
3. **캡셔닝 효과를 제대로 검증할 질의셋.** SDS KoPub 600문항은 정답이 IMAGE 경로인 문항이 1개뿐이라
   통계적 검증이 안 된다. IMAGE 경로 997장을 대상으로 직접 질의를 만들거나, DART 2차 코퍼스에서
   표·차트 밀도가 높은 문서를 의도적으로 골라야 한다.
4. **DART 2차 코퍼스.** 다년도 사업보고서로 방해 문서와 스캔 첨부를 추가한다. 3번과 겸해서 진행.
5. **Phase 5 마무리.** 비용 분석(현행 API 모델 가격 기준, 전기료·감가 포함), README 아키텍처 다이어그램.
6. **선택.** OCR 경로 1,387장 2차 캡셔닝(표 구조 보존 목적), ColPali 비교(8GB에서는 어려움).
