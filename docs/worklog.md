...
- 기권 게이트 설계 시도 (D-22, 부정 결과): 리랭크 1위 점수와 판정 등급을 42문항으로 대조하니 상관이 거의
  없었다(정답이 0.07점, 오답이 0.99점인 사례 존재). 단일 점수 임계값 게이트는 도입하지 않기로 결정
...
1. **기권 게이트, 다른 신호로 재시도.** D-22에서 리랭크 1위 점수 단독은 기각됐다. 다음 후보는 1·2위 점수
   격차(margin), 또는 top-5 안 회사/문서 다양성. D-13 부작용(검색 실패 시 무관 문서로 답변)이 아직 남아 있다.
2. **생성 품질 재채점.** D-11~D-13 수정 후 15문항을 temperature 0으로 재생성해 기존 기준으로 다시 채점.
3. **SDS 판정(42문항) 재채점.** D-20/D-21로 자동 지표는 다시 쟀지만 수동 판정은 D-19 그대로다.
