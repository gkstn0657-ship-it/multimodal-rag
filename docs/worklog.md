...
1. **BM25 하이브리드 검색.** D-19의 DART 실패(회사명·연도 불일치)를 겨냥. 벡터 top-10 + BM25 top-10을
   RRF로 합쳐 리랭커에 넘긴다. 합성 185건·DART 판정 10문항으로 재측정.
2. **회사·연도·문서명 접두어(Contextual Retrieval 축소판).** 1번 효과를 본 뒤 결정. 재임베딩 20분.
3. **기권 게이트.** D-13 부작용(검색 실패 시 무관 문서로 답변). 판정 평가의 리랭크 점수 분포로 임계값을 정한다.
4. **생성 품질 재채점.** D-11~D-13 수정 후 15문항을 temperature 0으로 재생성해 기존 기준으로 다시 채점.
5. **차트가 있는 TEXT 경로 페이지.** DART 원본 PDF의 `fitz.get_drawings()`로 재시도 — 선택.
6. **ColQwen2 대조 실험.** IMAGE 경로 997장만 이미지 직접 임베딩해 캡션 저장소와 정면 비교 — 선택.
7. **README·다이어그램 마무리.**
