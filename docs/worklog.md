...
**judge가 필요한 생성 품질 평가는 미착수**: 이 환경에 피평가 VLM과 분리된 LLM judge(claude CLI 등)가
없어, 자기 채점(self-judging) 문제를 피할 방법이 마땅치 않다. 다음 세션에서 API 예산을 확보하거나
사람이 직접 채점하는 방식으로 진행해야 한다.
...
커밋 26개. 각 단계는 `feat` / `fix` / `perf` / `docs` / `test` / `chore` 접두어로 나뉘어 있다.
...
1. **Phase 3 생성 품질 평가.** 검색이 아니라 최종 답변 품질을 judge로 채점하는 부분이 남았다.
   이 환경에 피평가 VLM(qwen2.5vl)과 분리된 LLM judge가 없다는 게 걸림돌이다. Claude API 예산을
   확보하거나, 사람이 직접 20~30문항을 채점하는 방식으로 진행해야 한다.
2. **캡셔닝 효과를 제대로 검증할 질의셋.** SDS KoPub 600문항은 정답이 IMAGE 경로인 문항이 1개뿐이라
   통계적 검증이 안 된다. IMAGE 경로 997장을 대상으로 직접 질의를 만들거나, DART 2차 코퍼스에서
   표·차트 밀도가 높은 문서를 의도적으로 골라야 한다.
3. **DART 2차 코퍼스.** 다년도 사업보고서로 방해 문서와 스캔 첨부를 추가한다. 2번과 겸해서 진행.
4. **Phase 5 마무리.** 비용 분석(현행 API 모델 가격 기준, 전기료·감가 포함), README 아키텍처 다이어그램.
5. **선택.** OCR 경로 1,387장 2차 캡셔닝(표 구조 보존 목적), ColPali 비교(8GB에서는 어려움).
