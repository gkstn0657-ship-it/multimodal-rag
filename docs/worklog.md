...
1. **회사·연도·문서명 접두어(Contextual Retrieval 축소판).** D-20에서도 DART 무관 비율(0.62)은 그대로라
   하이브리드로 못 잡은 "같은 회사, 다른 주제" 오류를 겨냥. 재임베딩 20분.
2. **기권 게이트.** D-13 부작용(검색 실패 시 무관 문서로 답변). 판정 평가의 리랭크 점수 분포로 임계값을 정한다.
3. **생성 품질 재채점.** D-11~D-13 수정 후 15문항을 temperature 0으로 재생성해 기존 기준으로 다시 채점.
4. **SDS 판정(42문항) 재채점.** D-20 하이브리드로 자동 지표는 다시 쟀지만 수동 판정은 D-19 그대로다.
